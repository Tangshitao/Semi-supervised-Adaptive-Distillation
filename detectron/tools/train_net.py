...
from core.config import teacher_cfg,cfg,student_cfg
from core.config import get_output_dir
from core.config import merge_cfg_from_file
from core.config import merge_cfg_from_list
from core.config import register_teacher,register_student,register_cfg,restore_C
from datasets.roidb import combined_roidb_for_training,combined_label_for_traning
...
def parse_args():
    ...
    parser.add_argument(
        ...
        help='Config file of teacher for training (and optionally testing)',
        default=None,
        type=str
    )
    parser.add_argument(
...
def main():
    ...
    if args.teacher_cfg_file:
        _cfg=copy.deepcopy(cfg)
        merge_cfg_from_file(args.teacher_cfg_file)
        assert_and_infer_cfg()
        logger.info('Teacher config:')
        logger.info(pprint.pformat(args.teacher_cfg_file))
        register_teacher(cfg)
        register_cfg(_cfg)
    restore_C()
...
def setup_model_for_training(model, output_dir):
    ...
    if cfg.TRAIN.WEIGHTS:
        ...
        nu.initialize_gpu_from_weights_file(model, cfg.TRAIN.WEIGHTS, gpu_id=0,train=True)
...
def add_model_training_inputs(model):
    ...
    if cfg.DISTILLATION.UNLABEL_DISTILLATION:
        unlabel_roidb=combined_roidb_for_training(
            cfg.DISTILLATION.UNLABEL_DATASETS,cfg.TRAIN.PROPOSAL_FILES
        )

    logger.info('{:d} roidb entries'.format(len(roidb)))
    model_builder.add_training_inputs(model, roidb=roidb,unlabel_roidb=unlabel_roidb)
