...
teacher_cfg=AttrDict()
student_cfg=AttrDict()
...
__C.TRAIN.OHEM_ROI_PER_IMG=128

__C.TRAIN.RFCN=False
...
__C.MODEL.BBOX_REG_WEIGHT_SCALE=1.0
...
__C.RETINANET.FINAL_KERNEL_SIZE=3

__C.RETINANET.L2_LOSS=False
...
__C.FPN.RFCN_PS_GRID_SIZE=[7,7,7,7]
...
__C.PIXEL_STD=np.array([[[1.0,1.0,1.0]]])
...
__C.DISTILLATION=AttrDict()

__C.DISTILLATION.DISTILLATION_ON=False

__C.DISTILLATION.LOSS_ALPHA=0.0

__C.DISTILLATION.LOSS_GAMMA=0.0

__C.DISTILLATION.LOSS_BETA=0.0

# Anchor label to be ignored during training
__C.DISTILLATION.IGNORED_LABEL=-1

# Distillation temperature
__C.DISTILLATION.TEMPERATURE=1.0

# If set to False, focal distillation loss is normalized by the same term as focal loss
__C.DISTILLATION.ADAPTIVE_NORMALIZER=False

# The power of normalizer
__C.DISTILLATION.LOGITS_POWER=1.0

# Unlabeled dataset is used
__C.DISTILLATION.UNLABEL_DISTILLATION=False

# Unlabeled dataset name
# The model uses TRAIN.DATASETS and UNLABELD_DATASETS for alternate training
__C.DISTILLATION.UNLABEL_DATASETS=()
# ---------------------------------------------------------------------------- #
# Deprecated options
...
def register_student(_cfg):
    global student_cfg
    student_cfg.update(copy.deepcopy(_cfg))

def register_teacher(_cfg):
    global teacher_cfg
    teacher_cfg.update(copy.deepcopy(_cfg))

def register_cfg(_cfg):
    global cfg
    cfg.update(copy.deepcopy(_cfg))

def switch_to_student():
    global cfg
    cfg.update(copy.deepcopy(student_cfg))

def switch_to_teacher():
    global cfg
    cfg.update(copy.deepcopy(teacher_cfg))


def test():
    print(teacher_cfg.MODEL.TYPE)

def restore_C():
    global __C
    __C.update(student_cfg)