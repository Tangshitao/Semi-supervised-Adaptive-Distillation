...
def im_detect_bbox(model, im, timers=None,img_path=None):
    ...
    if img_path is not None:
        main_dir='/mnt/lustre/tangshitao/dataset/coco/distillation/teacher'
        sub_img_path=img_path.split('/')[-1][:-4]
        path=os.path.join(main_dir,sub_img_path+'.pkl')
        cp.dump({'cls_probs':cls_probs,"box_preds":box_preds},open(path,'w'))
