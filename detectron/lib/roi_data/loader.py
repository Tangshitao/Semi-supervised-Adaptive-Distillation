...
class RoIDataLoader(object):
    def __init__(
        ...
        self._roidb = roidb
        self._roidb_tmp=unlabel_roidb
        self._lock = threading.Lock()
        self._perm = deque(range(len(self._roidb)))
        if unlabel_roidb is not None:
            self._perm_tmp=deque(range(len(self._roidb_tmp)))
            self._unlabel_minibatch_queue=Queue.Queue(maxsize=minibatch_queue_size)
        ...
        if self._roidb_tmp is not None:
            self._shuffle_unlabel_roidb_inds()
        self.create_threads()
    ...
    def _shuffle_unlabel_roidb_inds(self):
        if cfg.TRAIN.ASPECT_GROUPING:
            widths = np.array([r['width'] for r in self._roidb_tmp])
            heights = np.array([r['height'] for r in self._roidb_tmp])
            horz = (widths >= heights)
            ...
            self._perm_tmp = np.random.permutation(np.arange(len(self._roidb_tmp)))
        self._perm_tmp = deque(self._perm_tmp)
        self._cur_tmp = 0
    ...
    def _get_next_unlabel_minibatch_inds(self):
        with self._lock:
            ...
            db_inds = [self._perm_tmp[i] for i in range(cfg.TRAIN.IMS_PER_BATCH)]
            self._perm_tmp.rotate(-cfg.TRAIN.IMS_PER_BATCH)
            self._cur_tmp += cfg.TRAIN.IMS_PER_BATCH
            if self._cur_tmp >= len(self._perm_tmp):
                self._shuffle_unlabel_roidb_inds()
        return db_inds
