...
from modeling.ResNet import add_stage
...
def add_fast_rcnn_losses(model):
    ...
    if cfg.TRAIN.OHEM:
        cls_prob_per_roi,loss_cls_per_roi=model.net.SoftmaxWithLossOHEM(
            ['cls_score', 'labels_int32'],['cls_prob_per_roi','loss_cls_per_roi']
        )
        ...
        per_roi_loss=model.net.Add(
            ['loss_cls_per_roi','loss_bbox_per_roi'],
            'per_roi_loss'
        )
    ...
    cls_prob, loss_cls = model.net.SoftmaxWithLoss(
        ['cls_score', 'labels_int32','label_weights'] if cfg.TRAIN.OHEM else  ['cls_score', 'labels_int32'], ['cls_prob', 'loss_cls'],
...
def add_rfcn_head(model,blob_in,dim_in,spatial_scale):
    model.PSRoIFeatureTransform(
        blob_in,
        'psroi_feat',
        blob_rois='rois',
        dim_in=dim_in,
...
def add_resnet_head(model,blob_in,dim_in,spatial_scale):
    hidden_dim = cfg.FAST_RCNN.MLP_HEAD_DIM
    roi_size = cfg.FAST_RCNN.ROI_XFORM_RESOLUTION
    roi_feat = model.RoIFeatureTransform(
        blob_in,
    ...
    return final_feat,int(dim_in*roi_size*roi_size/4)