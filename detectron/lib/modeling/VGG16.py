...
def add_VGG16_fc(model,blob_in,dim_in,is_test):
    model.MaxPool(blob_in,'pool5',kernel=2,stride=2)
    model.FC('pool5', 'fc6', dim_in * 7 * 7, 4096)
    model.Relu('fc6', 'fc6')
    model.Dropout('fc6','fc6drop',ratio=0.5,is_test=is_test)
    model.FC('fc6drop', 'fc7', 4096, 4096)
    blob_out = model.Relu('fc7', 'fc7')
    model.Dropout('fc7','fc7drop',ratio=0.5,is_test=is_test)
    model.FC('fc7drop','cls_score',4096,1000)
    return blob_out

def add_VGG16_cls(model):
    add_VGG16_conv5_body(model)
    add_VGG16_fc(model,'conv5_3',512,not cfg.TRAIN)
