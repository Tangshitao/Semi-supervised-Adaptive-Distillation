...
def add_distill_loss(model,student_prefix,teacher_prefix):
    ...
    if cfg.DISTILLATION.ADAPTIVE_NORMALIZER:
            ...
            model.net.PowSum(cls_lvl_logits_power_sum_list,student_prefix+'distill_normalizer',power=cfg.DISTILLATION.LOGITS_POWER)
    ...
    for lvl in range(k_min, k_max + 1):
        ...
        cls_adaptive_distill_loss = model.net.SigmoidAdaptiveDistillLoss(
                                [
                                    student_prefix+cls_lvl_logits,teacher_prefix+'retnet_cls_prob_' + suffix, 'retnet_cls_labels_' + suffix,
                                    student_prefix+('retnet_fg_num' if not cfg.DISTILLATION.ADAPTIVE_NORMALIZER else 'distill_normalizer')
                                ],
                                [student_prefix+'fl_distill_{}'.format(suffix)],
                                gamma=cfg.DISTILLATION.LOSS_GAMMA,
                                alpha=cfg.DISTILLATION.LOSS_ALPHA,
                                scale=model.GetLossScale()*cfg.DISTILLATION.TEMPERATURE*cfg.DISTILLATION.TEMPERATURE,
