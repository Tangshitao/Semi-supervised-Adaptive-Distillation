...
def add_ResNet101_conv5_body(model,final_conv=False):
    return add_ResNet_convX_body(model, (3, 4, 23, 3),final_conv=final_conv)
...
def add_ResNet_convX_body(model, block_counts, freeze_at=2,final_conv=False):
    ...
    dim_in = 64
    dim_bottleneck = int(cfg.RESNETS.NUM_GROUPS * cfg.RESNETS.WIDTH_PER_GROUP*cfg.RESNETS.CHANNEL_RATIO)

    (n1, n2, n3) = block_counts[:3]
    s, dim_in = add_stage(model, 'res2', p, n1, dim_in, int(256*cfg.RESNETS.CHANNEL_RATIO), dim_bottleneck, 1)
    if freeze_at == 2:
        model.StopGradient(s, s)
    s, dim_in = add_stage(
        model, 'res3', s, n2, dim_in, int(512*cfg.RESNETS.CHANNEL_RATIO), dim_bottleneck * 2, 1
    )
    if freeze_at == 3:
        model.StopGradient(s, s)
    s, dim_in = add_stage(
        model, 'res4', s, n3, dim_in, int(1024*cfg.RESNETS.CHANNEL_RATIO), dim_bottleneck * 4, 1
    )
    if freeze_at == 4:
        model.StopGradient(s, s)
    if len(block_counts) == 4:
        n4 = block_counts[3]
        s, dim_in = add_stage(
            model, 'res5', s, n4, dim_in, int(2048*cfg.RESNETS.CHANNEL_RATIO), dim_bottleneck * 8,
            cfg.RESNETS.RES5_DILATION
        )
        if freeze_at == 5:
            model.StopGradient(s, s)
        if final_conv:
            dim_out=512*cfg.RESNETS.CHANNEL_RATIO
            s=model.Conv(s,'conv_new1',dim_in,dim_out,3,pad=6,stride=1,no_bias=1,dilation=6)
            dim_in=dim_out
