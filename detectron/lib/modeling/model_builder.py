...
from core.config import cfg,student_cfg, switch_to_student,switch_to_teacher
...
def retinanet(model,teacher_scope=False):
    # TODO(rbg): fold into build_generic_detection_model
    return build_generic_retinanet_model(model, get_func(cfg.MODEL.CONV_BODY),teacher_scope=teacher_scope)

def distillation(model):
    return build_generic_retinanet_model_dissstillation(model,get_func(cfg.MODEL.CONV_BODY))
...
def create(model_type_func, train=False, gpu_id=0):
    ...
    train_model=get_func(model_type_func)(model)
    
    return train_model
    
def create_distill(model_type_func,teacher_model_type_func,train=False,gpu_id=0):
    switch_to_teacher()

    model = DetectionModelHelper(
        name=teacher_model_type_func+'_teacher',
        train=False,
        num_classes=cfg.MODEL.NUM_CLASSES,
        init_params=False
    )
    model.only_build_forward_pass = False
    model.target_gpu_id = gpu_id
    teacher_model=get_func(teacher_model_type_func)(model,True)
    switch_to_student()
    ...
    return train_model,teacher_model
...
def build_generic_rfcn_model(model, add_conv_body_func, dim_reduce=None):
    ...
    def _single_gpu_build_func(model):
        ...
        with name and device scoping to create a data parallel model."""
        blob, dim, spatial_scale = add_conv_body_func(model,final_conv=True)
        if not model.train:
            model.conv_body_net = model.net.Clone('conv_body_net')
        rfcn_heads.add_rfcn_outputs(model, blob, dim, spatial_scale,dim_reduce)
        if model.train:
            loss_gradients = fast_rcnn_heads.add_fast_rcnn_losses(model)
...
def build_generic_retinanet_model(
    model, add_conv_body_func, freeze_conv_body=False,teacher_scope=False
):
    # TODO(rbg): fold this function into build_generic_detection_model
    def _single_gpu_build_func(model):
        """Builds the model on a single GPU. Can be called in a loop over GPUs
    ...
    optim.build_data_parallel_model(model, _single_gpu_build_func,teacher_scope)
    return model
...
def add_training_inputs(model, roidb=None,unlabel_roidb=None):
    ...
    *after* model_builder.create().
    ...
    if roidb is not None:
        ...
        model.roi_data_loader = RoIDataLoader(
            roidb,unlabel_roidb=unlabel_roidb, num_loaders=cfg.DATA_LOADER.NUM_THREADS
        )
