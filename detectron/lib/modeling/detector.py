...
class DetectionModelHelper(cnn.CNNModelHelper):
    ...
    def PSRoIFeatureTransform(
        ...
        if isinstance(blobs_in, list):
            ...
            for lvl in range(k_min, k_max + 1):
                #conv_cls = conv_clss[k_max - lvl]  # blobs_in is in reversed order
                #conv_bbox_pred=conv_clss[k_max - lvl]
                bls_in=blobs_in[k_max - lvl]
                sc = spatial_scale[k_max - lvl]  # in reversed order
                bl_rois = blob_rois + '_fpn' + str(lvl)
                cls_bl_out,bbox_bl_out = add_rfcn_heads(bls_in,bl_rois,sc,num_bbox_reg_classes,lvl-k_min)
                cls_bl_out_list.append(cls_bl_out)
                bbox_bl_out_list.append(bbox_bl_out)
                
            # The pooled features from all levels are concatenated along the
            # batch dimension into a single 4D tensor.
            cls_xform_shuffled, _ = self.net.Concat(
                cls_bl_out_list, [blob_out + '_cls_shuffled', 'cls_concat_' + blob_out],
                axis=0
            )
            bbox_xform_shuffled, _ = self.net.Concat(
                bbox_bl_out_list, [blob_out + '_bbox_shuffled', 'bbox_concat_' + blob_out],
                axis=0
            )
            # Unshuffle to match rois from dataloader
            restore_bl = blob_rois + '_idx_restore_int32'
            self.net.BatchPermutation(
                [cls_xform_shuffled, restore_bl], 'cls_score'
            )
            self.net.BatchPermutation(
                [bbox_xform_shuffled, restore_bl], 'bbox_pred'
            )
        else:
            # sampling_ratio is ignored for RoIPoolF
    ...
    def GetBBoxLossScale(self):
        return cfg.MODEL.BBOX_REG_WEIGHT_SCALE/cfg.NUM_GPUS
