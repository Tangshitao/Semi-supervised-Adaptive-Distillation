...
def im_list_to_blob(ims,max_shape=None):
...
def preprocess_im(im,pixel_means,pixel_div,pixel_std):
...
def prep_im_for_teacher_blob(im,pixel_means,h,w):
    im=preprocess_im(im,teacher_cfg.PIXEL_MEANS,teacher_cfg.PIXEL_DIV,teacher_cfg.PIXEL_STD)
    im=cv2.resize(im,(w,h),interpolation=cv2.INTER_LINEAR)
    return im

def prep_im_for_blob(im, pixel_means,pixel_div,pixel_std, target_sizes, max_size):
    ...
    """
    im=preprocess_im(im,pixel_means,pixel_div,pixel_std)
    im_shape = im.shape
...
def prep_im_for_blob_cls(im,pixel_means, crop_size,is_test):
    im_shape = im.shape
    im_size_min = np.min(im_shape[0:2])
    im_size_max = np.max(im_shape[0:2])
    ...
    start_h=random.randint(0,im.shape[0]-crop_size)
    start_w=random.randint(0,im.shape[1]-crop_size)
    v=random.random()
    if v<0.5:
        im=im[:,::-1,:]
    im = im[start_h:start_h+crop_size,start_w:start_w+crop_size,:]
