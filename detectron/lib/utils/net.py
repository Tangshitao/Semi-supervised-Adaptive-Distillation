...
from core.config import cfg,teacher_cfg
...
def initialize_gpu_from_weights_file(model, weights_file, gpu_id=0, train=False):
    ...
    if cfg.DISTILLATION.DISTILLATION_ON and train:
        logger.info('Loading teacher weights from: {}'.format(teacher_cfg.TRAIN.WEIGHTS))
        with open(teacher_cfg.TRAIN.WEIGHTS,'r') as f:
            teacher_blobs=pickle.load(f)
            if 'blobs' in teacher_blobs:
                teacher_blobs=teacher_blobs['blobs']
            for k,v in teacher_blobs.items():
                src_blobs['teacher/{}'.format(k)]=v
        ...
        for unscoped_param_name in unscoped_param_names.keys():
            ...
            if dst_name in ws_blobs:
                ...
                if ws_blob.shape != src_blobs[src_name].shape:
                    logger.info("Shape missmatch: name: {} src: {}, dst: {}".format(dst_name,ws_blob.shape,src_blobs[src_name].shape))
                    continue
...
def sum_multi_gpu_blob(blob_name):
    ...
    for i in range(cfg.NUM_GPUS):
        #a=float(workspace.FetchBlob('gpu_{}/{}'.format(i, blob_name)))
...
def print_net(model, namescope='gpu_0'):
    ...
    for op in op_list:
        ...
        if namescope is None or output_name.startswith(namescope):
            ...
            output = workspace.FetchBlob(output_name)
