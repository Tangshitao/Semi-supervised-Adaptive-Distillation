...
def combined_roidb_for_training(dataset_names, proposal_files, _filter=True):
...
def combined_label_for_traning(dataset_names):
    def get_label(dataset_name):
        ds = ImageListDataset(dataset_name)
        return ds.image_list
    image_lists=[get_label(args) for args in dataset_names]
    image_list=image_lists[0]
    for r in image_lists:
        image_list.extend(r)
    return image_list
