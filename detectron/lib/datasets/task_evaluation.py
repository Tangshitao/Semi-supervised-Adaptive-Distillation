...
import datasets.vid_dataset_evaluator as vid_dataset_evaluator
...
def evaluate_boxes(dataset, all_boxes, output_dir, use_matlab=False):
    ...
    elif _use_vid_evaluator(dataset):
        vid_eval = vid_dataset_evaluator.evaluate_boxes(
            dataset, all_boxes, output_dir, use_salt=not_comp, cleanup=False
        )
        box_results = _coco_eval_to_box_results(vid_eval)
    else:
...
def _use_vid_evaluator(dataset):
    return dataset.name[:3] == 'vid'
